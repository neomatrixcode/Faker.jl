...
for name, sym in [('years', 'y'), ('weeks', 'w'), ('days', 'd'), ('hours', 'h'), ('minutes', 'm'), ('seconds', 's')]:
    ...
    date_time_ad():
        ...
        datetime.fromtimestamp(0) + timedelta(seconds=ts)
    ...
    time(, pattern='%H:%M:%S'):
        ...
        :example '15:02:34'
        """
        .date_time().time().strftime(pattern)

    _parse_date_time(, text):
        if isinstance(text, (datetime, date, real_datetime, real_date)):
            datetime_to_timestamp(text)
        now = datetime.now()
        if isinstance(text, timedelta):
            datetime_to_timestamp(now - text)
        if is_string(text):
            if text == 'now':
            ...
            datetime_to_timestamp(now + timedelta(**time_params))
        if isinstance(text, int):
            datetime_to_timestamp(now + timedelta(text))
    ...
    date_time_between(, start_date='-30y', end_date='now'):
        ...
        :example DateTime('1999-02-02 11:42:52')
         DateTime
        """
        start_date = ._parse_date_time(start_date)
        end_date = ._parse_date_time(end_date)
        timestamp = random.randint(start_date, end_date)
        datetime.fromtimestamp(timestamp)

    date_time_between_dates(, datetime_start=None, datetime_end=None):
        """
        Takes two DateTime objects ands a random date between the two given dates.
    ...
    date_time_this_century(, before_now=True, after_now=False):
        ...
        if before_now and after_now:
            .date_time_between_dates(this_century_start, next_century_start)
        elif not before_now and after_now:
            .date_time_between_dates(now, next_century_start)
        elif not after_now and before_now:
            .date_time_between_dates(this_century_start, now)
        else:
            now
    ...
    date_time_this_decade(, before_now=True, after_now=False):
        ...
        if before_now and after_now:
            .date_time_between_dates(this_decade_start, next_decade_start)
        elif not before_now and after_now:
            .date_time_between_dates(now, next_decade_start)
        elif not after_now and before_now:
            .date_time_between_dates(this_decade_start, now)
        else:
            now
    ...
    date_time_this_year(, before_now=True, after_now=False):
        ...
        if before_now and after_now:
            .date_time_between_dates(this_year_start, next_year_start)
        elif not before_now and after_now:
            .date_time_between_dates(now, next_year_start)
        elif not after_now and before_now:
            .date_time_between_dates(this_year_start, now)
        else:
            now
    ...
    date_time_this_month(, before_now=True, after_now=False):
        ...
        if before_now and after_now:
            .date_time_between_dates(this_month_start, next_month_start)
        elif not before_now and after_now:
            .date_time_between_dates(now, next_month_start)
        elif not after_now and before_now:
            .date_time_between_dates(this_month_start, now)
        else:
            now
    ...
    century():
        ...
        :example 'XVII'
        """
        .random_element(.centuries)

    timezone():
        random.choice(.random_element(.countries)['timezones'])


zip_codes[p][rand(1:2)]