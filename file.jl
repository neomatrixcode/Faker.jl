include("base.jl")
    ...
    function mime_type(category="None")  

        mime_types = Dict(
        "application"=>application_mime_types[rand(1:length(application_mime_types))] ,
        "audio"=>audio_mime_types[rand(1:length(audio_mime_types))],
        "image"=>image_mime_types[rand(1:length(image_mime_types))],
        "message"=>message_mime_types[rand(1:length(message_mime_types))],
        "model"=>model_mime_types[rand(1:length(model_mime_types))],
        "multipart"=>multipart_mime_types[rand(1:length(multipart_mime_types))],
        "text"=>text_mime_types[rand(1:length(text_mime_types))],
        "video"=>video_mime_types[rand(1:length(video_mime_types))] )

        if category == "None" 
        category= [key for key in  keys(mime_types)][rand(1:8)]
        end
        mime_types[category]
    end
    ...
    function file_extension( category="None")

       file_extensions = Dict(
        "audio"=>  audio_file_extenstions[rand(1:length(audio_file_extenstions))],
        "image"=>  image_file_extenstions[rand(1:length(image_file_extenstions))],
        "text"=>  text_file_extensions[rand(1:length(text_file_extensions))],
        "video"=>  video_file_extensions[rand(1:length(video_file_extensions))]
    )
        #:param category: audio|image|text|video
       if category == "None" 
        category= [key for key in  keys(file_extensions)][rand(1:4)]
        end
  ...
  function file_name( category="None", extencion="None")
       
        #:param category: audio|image|text|video
        #:param extension: file extension
       
    if extencion == "None" 
     extencion = file_extension(category)
    end 
