    ...
    company_prefix() = company_preffixes[rand(1:length(company_preffixes))]
     ...
     catch_phrase()= (resultado ="";
        for word_list in catch_phrase_words
            resultado *= ("$(word_list[rand(1:length(word_list))]) ")
        end;
        resultado)


     bs()=(resultado ="";
        for word_list in bsWords
            resultado *= ("$(word_list[rand(1:length(word_list))]) ")
        end;
        resultado)


    company()=formatscomp[rand(1:length(formatscomp))]
