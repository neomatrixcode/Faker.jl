    ...
    date_time_ad()= replace("$(iso8601())","T"," ")
   ...
   function time(pattern="%H:%M:%S")
    ...
    function date_time_between(start_date="-30y", end_date="now")
        ...
        start_date = DateTime(now()) + Dates.Year(parse(Int,replace(start_date,"y","")))
        end_date = DateTime(now()) + Dates.Year(parse(Int,replace(end_date,"y","")))

         dr=(start_date : end_date)
        return replace("$(dr[rand(1:(length(dr)))])","T"," ") 
   ...
   function date_time_this_year( before_now=true, after_now=false) 
    ...
    function timezone()
	    ...
	    resul = timeszon[rand(1:length(timeszon))]["timezones"]
